    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 dark:from-slate-900 via-slate-100 dark:via-slate-800 to-slate-200 dark:to-slate-900 py-10 px-4">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-md dark:shadow-2xl rounded-2xl p-6 flex flex-col gap-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">Purchase & Sales</h2>
              <p className="text-slate-600 dark:text-slate-400 text-sm">Enter batches in chronological order</p>
            </div>
            <button
              onClick={handleReset}
              ...
              {batches.map((batch, index) => (
                <div key={batch.id} className="flex gap-3 items-end">
                  <div className="flex-1">
                    <label className="block text-xs text-slate-600 dark:text-slate-400 mb-1">Qty (Batch {index + 1})</label>
                    <input
                      type="number"
                      value={batch.qty}
                      ...
                      placeholder="0"
                      min={0}
                      className="w-full px-3 py-2.5 rounded-xl bg-transparent dark:bg-transparent dark:bg-slate-900/50 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
                    />
                  </div>
                  <div className="flex-1">
                    <label className="block text-xs text-slate-600 dark:text-slate-400 mb-1">Cost / Unit</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        min={0}
                        step="any"
                        className="w-full pl-8 pr-3 py-2.5 rounded-xl bg-transparent dark:bg-transparent dark:bg-slate-900/50 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
                      />
                    </div>
                  </div>
                  <button
          ...
          <div className="space-y-4 pt-4">
            <h3 className="text-sm font-bold text-amber-400 uppercase tracking-wider flex items-center gap-2 border-b border-gray-200 dark:border-white/10 pb-2">
              <Layers className="w-4 h-4"/> Sales / Usage (Out)
            </h3>
            <div>
              <label className="block text-[13px] text-slate-700 dark:text-slate-300 mb-1">Quantity Sold</label>
              <input
                type="number"
                value={quantitySold}
                ...
                placeholder="0"
                min={0}
                className="w-full px-4 py-3 rounded-xl bg-transparent dark:bg-transparent dark:bg-slate-900/50 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all text-lg font-semibold"
              />
            </div>
          </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-md dark:shadow-2xl rounded-2xl p-6">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="text-lg font-bold text-slate-900 dark:text-white">FIFO Analysis</h2>
            ...
            {results ? (
              <>
                <div className="border border-red-500/30 rounded-2xl p-5 mb-4 text-center bg-gradient-to-r from-red-600/10 to-rose-600/10">
                  ...
                  <p className="text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">{disp(results.cogs)}</p>
                ...
                <div className="grid grid-cols-2 gap-3 mb-5">
                  <div className="rounded-xl border bg-gray-50 dark:bg-white dark:bg-gray-50 dark:bg-slate-800/50 border-gray-100 dark:border-gray-100 dark:border-white/5 p-4 flex flex-col gap-1">
                    <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-400 flex items-center gap-1"><Package className="w-3 h-3"/> Remaining Qty</div>
                    <p className="font-bold text-xl text-slate-900 dark:text-white">{results.remainingQty} <span className="text-sm font-normal text-slate-500">units</span></p>
                  </div>
                ...
                {/* FIFO TABLE */}
                <h3 className="text-sm font-bold text-slate-900 dark:text-white mb-3">Batch Status (Remaining)</h3>
      ...
      {results && (
        <div className="max-w-6xl mx-auto mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Bar Chart */}
          <div className="bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-md dark:shadow-2xl rounded-2xl p-6">
            <h3 className="text-base font-bold text-slate-900 dark:text-white mb-6">Quantity Flow</h3>
          ...
          <div className="bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-md dark:shadow-2xl rounded-2xl p-6 flex flex-col">
            <h3 className="text-base font-bold text-slate-900 dark:text-white mb-2">Cost Breakdown</h3>
