  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 dark:from-slate-900 via-slate-100 dark:via-slate-800 to-green-100 dark:to-green-950 py-10 px-4">
      <div className="max-w-6xl mx-auto mb-8 text-center">
        ...
        <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-2">Remittance Fee <span className="text-green-400">Calculator</span></motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-slate-600 dark:text-slate-400 text-lg">Calculate transfer fees and know exactly how much your recipient receives.</motion.p>
      </div>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }} className="bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-md dark:shadow-2xl rounded-2xl p-6 flex flex-col gap-4">
          ...
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Currency Pair</label>
            <div className="flex items-center gap-2">
              ...
              <select value={toCcy} onChange={e => setToCcy(e.target.value)} className="flex-1 px-4 py-2.5 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-green-500">{CURRENCIES.map(c => <option key={c} value={c} className="bg-white dark:bg-slate-800">{c}</option>)}</select>
            </div>
          </div>
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Amount to Send ({NAMES[fromCcy]})</label>
            <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-400 text-sm">{fromSym}</span>
              <input type="number" value={amountRaw} onChange={e => setAmountRaw(e.target.value)} min={0} className="w-full pl-8 pr-4 py-3 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-green-500 transition-all" /></div>
          </div>
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Transfer Fee Type</label>
            <div className="grid grid-cols-2 gap-2">
              {[{ id: 'flat', label: 'Flat Fee' }, { id: 'percentage', label: 'Percentage (%)' }].map(t => <button key={t.id} onClick={() => setFeeType(t.id as 'flat' | 'percentage')} className={`py-2.5 rounded-xl border text-xs font-semibold transition-all ${feeType === t.id ? 'bg-green-600 border-green-500 text-white' : 'bg-white dark:bg-white/5 border-gray-200 dark:border-white/10 text-slate-600 dark:text-slate-400 hover:border-green-500/50'}`}>{t.label}</button>)}
            </div>
          </div>
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Fee Value {feeType === 'flat' ? `(${fromSym})` : '(%)'}</label>
            <div className="relative"><input type="number" value={feeValueRaw} onChange={e => setFeeValueRaw(e.target.value)} min={0} step={feeType === 'percentage' ? '0.1' : '1'} className="w-full pl-4 pr-10 py-3 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-green-500 transition-all" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-400 text-sm">{feeType === 'percentage' ? '%' : fromSym}</span></div>
          </div>
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Additional Charges ({fromSym}) <span className="text-slate-500 font-normal">optional</span></label>
          ...
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Exchange Rate Override <span className="text-slate-500 font-normal">optional — leave blank for live rate</span></label>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="flex flex-col gap-5">
          <div className="bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-md dark:shadow-2xl rounded-2xl p-6">
            <div className="flex items-center justify-between mb-5"><div><h2 className="text-lg font-bold text-slate-900 dark:text-white">Results</h2><p className="text-slate-600 dark:text-slate-400 text-sm">Rates: <span className="text-green-300">{relTime}</span></p></div>
            ...
            {result ? (
              <div className="flex flex-col gap-3">
                <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/30 rounded-xl p-4"><p className="text-xs font-semibold text-green-300 uppercase tracking-wider mb-1">Recipient Gets</p><p className="text-3xl font-extrabold text-slate-900 dark:text-white">{toSym}{result.receivedAmount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p><p className="text-xs text-slate-600 dark:text-slate-400 mt-1">1 {fromCcy} = {result.rate.toFixed(4)} {toCcy}</p></div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Transfer Fee</p><p className="text-xl font-bold text-red-300">{fmtAmt(result.fee, fromSym)}</p></div>
                  <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Total Charges</p><p className="text-xl font-bold text-amber-300">{fmtAmt(result.totalDeductions, fromSym)}</p></div>
                  <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Net Amount Sent</p><p className="text-xl font-bold text-slate-900 dark:text-white">{fmtAmt(result.netAmount, fromSym)}</p></div>
                  <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Additional</p><p className="text-xl font-bold text-slate-700 dark:text-slate-300">{fmtAmt(result.additionalCharges, fromSym)}</p></div>
                </div>
          ...
          {result && (
            <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-2xl overflow-hidden">
              <div className="px-5 py-3 border-b border-gray-200 dark:border-white/10"><p className="text-sm font-bold text-slate-900 dark:text-white">Transfer Breakdown</p></div>
                ...
                <tbody>
                  {[{ label: 'Sent Amount', val: fmtAmt(Number(amountRaw) || 0, fromSym) }, { label: 'Transfer Fee', val: fmtAmt(result.fee, fromSym) }, { label: 'Additional Charges', val: fmtAmt(result.additionalCharges, fromSym) }, { label: 'Net Sent', val: fmtAmt(result.netAmount, fromSym) }, { label: 'Received Amount', val: `${toSym}${result.receivedAmount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}` }].map((row, i) => (
                    <tr key={row.label} className={`border-t border-gray-100 dark:border-gray-100 dark:border-white/5 ${i % 2 === 0 ? '' : 'bg-white/[0.02]'} ${i === 4 ? 'bg-green-500/5 font-bold' : ''}`}>
                      <td className="px-4 py-2.5 text-slate-700 dark:text-slate-300">{row.label}</td><td className={`px-4 py-2.5 text-right ${i === 4 ? 'text-green-400 font-extrabold text-base' : 'text-slate-900 dark:text-white font-semibold'}`}>{row.val}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
