        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 flex flex-col gap-6">
          <div className="flex items-center justify-between border-b border-white/10 pb-4">
            <div>
              ...
              <p className="text-slate-400 text-sm">Define tax structure</p>
            </div>
            <button
              onClick={handleReset}
          ...
          <div className="space-y-4 pt-2">
            ...
            <div>
               <label className="block text-[13px] text-slate-300 mb-1">Calculation Type</label>
               <select
            ...
            <div>
              <label className="block text-[13px] text-slate-300 mb-1">{calculationType === 'exclusive' ? 'Base Amount (Excluding Tax)' : 'Total Amount (Including Tax)'}</label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-sm">{inputSym}</span>
                <input type="number" min={0} value={amount} onChange={e => setAmount(e.target.value)} className="w-full pl-8 pr-3 py-2.5 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-violet-500 transition-all font-semibold" />
              </div>
            </div>

            <div>
              <label className="block text-[13px] text-slate-300 mb-1">GST Rate Bracket (%)</label>
              <select
                  value={gstRate}
            ...
            {gstRate === 'custom' && (
              <div>
                <label className="block text-[13px] text-slate-300 mb-1">Custom GST Rate (%)</label>
                <div className="relative">
                  <input type="number" min={0} value={gstRate} onChange={e => setGstRate(e.target.value)} className="w-full pl-3 pr-8 py-2.5 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-violet-500 transition-all font-semibold" />
            ...
            {results ? (
              <div className="flex flex-col flex-1">
                <div className="border rounded-2xl p-5 mb-5 text-center bg-gradient-to-r from-violet-600/10 to-purple-600/10 border-violet-500/30">
                  ...
                  <p className="text-6xl font-extrabold text-white tracking-tight mb-2">
                    {disp(results.totalAmount)}
                  </p>
                  <div className="inline-flex items-center justify-center bg-black/20 rounded-full px-4 py-1 text-sm border border-white/5 mt-2">
                    <span className="text-slate-300 font-semibold mr-2">Tax Component:</span>
                ...
                <div className="grid grid-cols-2 gap-3 mb-5">
                  <div className="rounded-xl border bg-slate-800/50 border-white/5 p-4 flex flex-col gap-1 relative overflow-hidden">
                    ...
                    <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Base Pre-Tax Amount</div>
                    <p className="font-bold text-lg text-blue-300">{disp(results.originalAmount)}</p>
                  </div>
                  <div className="grid grid-cols-1 gap-3">
                     <div className="rounded-xl border bg-slate-800/50 border-white/5 p-2 flex justify-between items-center relative overflow-hidden">
                       <div className="absolute top-0 right-0 h-full w-1 bg-amber-500" />
                       <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 pl-2">CGST {(Number(gstRate || 0)/2)}%</div>
                       <p className="font-bold text-sm text-white pr-2">{disp(results.cgst)}</p>
                     </div>
                     <div className="rounded-xl border bg-slate-800/50 border-white/5 p-2 flex justify-between items-center relative overflow-hidden">
                       <div className="absolute top-0 right-0 h-full w-1 bg-pink-500" />
                       <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-400 pl-2">SGST {(Number(gstRate || 0)/2)}%</div>
                       <p className="font-bold text-sm text-white pr-2">{disp(results.sgst)}</p>
                     </div>
                  </div>
                </div>
      ...
      {results && (
        <div className="max-w-6xl mx-auto mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6">
            <h3 className="text-base font-bold text-white mb-6">Price Composition</h3>
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 flex flex-col">
            <h3 className="text-base font-bold text-white mb-2">Invoice Split</h3>
