  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 dark:from-slate-900 via-slate-100 dark:via-slate-800 to-sky-100 dark:to-sky-950 py-10 px-4">
      <div className="max-w-4xl mx-auto mb-8 text-center">
        ...
        <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-2">Currency <span className="text-sky-400">Converter</span></motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-slate-600 dark:text-slate-400 text-lg">Real-time currency conversion with live exchange rates.</motion.p>
      </div>

      <div className="max-w-4xl mx-auto flex flex-col gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-md dark:shadow-2xl rounded-2xl p-8">
          <div className="flex items-center justify-between mb-8">
            <div><h2 className="text-lg font-bold text-slate-900 dark:text-white">Converter</h2><p className="text-slate-600 dark:text-slate-400 text-sm">Updated: <span className="text-sky-300">{relTime}</span></p></div>
          ...
          {/* Amount Input */}
          <div className="mb-6"><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Amount</label>
            <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="100" min={0} className="w-full px-5 py-4 text-2xl font-bold rounded-2xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-sky-500 transition-all" />
          </div>
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1">
              ...
              <select value={from} onChange={e => setFrom(e.target.value)} className="w-full px-4 py-3 rounded-2xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white text-base font-semibold focus:outline-none focus:ring-2 focus:ring-sky-500 transition-all">
            ...
            <div className="flex-1">
              ...
              <select value={to} onChange={e => setTo(e.target.value)} className="w-full px-4 py-3 rounded-2xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white text-base font-semibold focus:outline-none focus:ring-2 focus:ring-sky-500 transition-all">
                {SUPPORTED.map(c => <option key={c} value={c} className="bg-white dark:bg-slate-800">{c} — {NAMES[c]}</option>)}
              </select>
            </div>
          </div>
          ...
          <motion.div key={`${converted}-${from}-${to}`} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-8 bg-gradient-to-r from-sky-500/20 to-indigo-500/20 border border-sky-500/30 rounded-2xl p-6 text-center">
            <p className="text-slate-600 dark:text-slate-400 text-sm mb-2">{amount || '0'} {from} =</p>
            <p className="text-4xl font-extrabold text-slate-900 dark:text-white">{SYMBOLS[to]}{converted.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 4 })}</p>
            <p className="text-sky-300 font-semibold text-lg mt-1">{to}</p>
            <div className="mt-3 pt-3 border-t border-gray-200 dark:border-white/10 text-slate-600 dark:text-slate-400 text-sm">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-md dark:shadow-2xl rounded-2xl p-6">
          <h3 className="text-base font-bold text-slate-900 dark:text-white mb-4">Popular Conversions {rates['INR'] ? `(${new Date().toLocaleDateString()})` : ''}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {popularPairs.map(([f, t]) => {
              const r = convert(1, f, t);
              return (
                <button key={`${f}-${t}`} onClick={() => { setFrom(f); setTo(t); }} className="flex items-center justify-between px-4 py-3 rounded-xl bg-white dark:bg-white/5 hover:bg-white/10 border border-gray-200 dark:border-white/10 hover:border-sky-500/50 transition-all text-left">
                  <span className="text-slate-900 dark:text-white font-semibold text-sm">1 {f}</span>
