  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 dark:from-slate-900 via-slate-100 dark:via-slate-800 to-indigo-100 dark:to-indigo-950 py-10 px-4">
      <div className="max-w-6xl mx-auto mb-8 text-center">
        ...
        <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-2">NPV <span className="text-indigo-400">Calculator</span></motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-slate-600 dark:text-slate-400 text-lg">Evaluate investment profitability using discounted cash flow analysis.</motion.p>
      </div>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }} className="bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-md dark:shadow-2xl rounded-2xl p-6 flex flex-col gap-4">
          <div className="flex items-center justify-between"><div><h2 className="text-lg font-bold text-slate-900 dark:text-white">Inputs</h2><p className="text-slate-600 dark:text-slate-400 text-sm">Enter investment details</p></div>
            <button onClick={() => { setInvestRaw('1000000'); setRateRaw('10'); setCashFlows([mkCF(), mkCF(), mkCF()]); }} className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:text-white bg-white dark:bg-white/5 hover:bg-white/10 border border-gray-200 dark:border-white/10 rounded-lg transition-all"><RotateCcw className="w-3.5 h-3.5" /> Reset</button>
          </div>
          ...
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Initial Investment</label>
            <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-400 text-sm">{inputSym}</span>
              <input type="number" value={investRaw} onChange={e => setInvestRaw(e.target.value)} min={0} className="w-full pl-8 pr-4 py-3 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all" /></div>
          </div>
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Discount Rate (% per year)</label>
            <div className="relative"><input type="number" value={rateRaw} onChange={e => setRateRaw(e.target.value)} min={0} max={100} step="0.1" className="w-full pl-4 pr-10 py-3 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-400 text-sm">%</span></div>
          </div>
          <div>
            <div className="flex items-center justify-between mb-2"><label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Yearly Cash Flows</label>
              ...
              <AnimatePresence>
                {cashFlows.map((cf, idx) => (
                  <motion.div key={cf.id} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="flex items-center gap-2">
                    ...
                    <div className="relative flex-1"><span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-400 text-xs">{inputSym}</span>
                      <input type="number" value={cf.amount} onChange={e => setCashFlows(p => p.map(x => x.id === cf.id ? { ...x, amount: e.target.value } : x))} placeholder="0" className="w-full pl-6 pr-2 py-2 rounded-lg bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500" /></div>
          ...
          <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-3"><p className="text-xs font-mono text-slate-600 dark:text-slate-400">NPV = Σ[CF/(1+r)ᵗ] − Initial Investment</p></div>
        </motion.div>
            ...
            {npv !== null ? (
              <div className="flex flex-col gap-3">
                <div className={`${npv >= 0 ? 'bg-emerald-500/20 border-emerald-500/30' : 'bg-red-500/20 border-red-500/30'} border rounded-xl p-4 text-center`}>
                  <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Net Present Value</p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Total PV of CFs</p><p className="text-xl font-bold text-indigo-300">{dispS(sumPV)}</p></div>
                  <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Initial Investment</p><p className="text-xl font-bold text-slate-700 dark:text-slate-300">{dispS(convertToINR(Number(investRaw) || 0, selectedInputCurrency))}</p></div>
                </div>
                <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-4">
                  <p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-3">Cash Flow Bars</p>
                  {rows.map((r, i) => (
                    <div key={r.year} className="mb-2">
                      <div className="flex justify-between text-xs mb-1"><span className="text-slate-600 dark:text-slate-400">Year {r.year}</span><span className="text-indigo-300">{disp(r.cf)}</span></div>
                      <div className="h-2.5 bg-white dark:bg-white/5 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${Math.abs(r.cf) / maxCF * 100}%` }} transition={{ duration: 0.7, delay: i * 0.05 }} className="h-full bg-indigo-500 rounded-full" /></div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {/* NPV Table */}
      {rows.length > 0 && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="max-w-6xl mx-auto mt-6 bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-md dark:shadow-2xl rounded-2xl p-6">
          <h3 className="text-base font-bold text-slate-900 dark:text-white mb-4">NPV Breakdown Table</h3>
          <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-white/10">
            <table className="w-full text-sm">
              <thead><tr className="bg-gray-50 dark:bg-white/5 text-slate-600 dark:text-slate-400 font-semibold"><th className="px-4 py-3 text-left">Year</th><th className="px-4 py-3 text-right">Cash Flow</th><th className="px-4 py-3 text-right">Discount Factor</th><th className="px-4 py-3 text-right">Present Value</th></tr></thead>
              <tbody>
                {rows.map((r, i) => <tr key={r.year} className={`border-t border-gray-100 dark:border-gray-100 dark:border-white/5 hover:bg-white/5 transition-colors ${i % 2 === 0 ? '' : 'bg-white/[0.02]'}`}>
                  <td className="px-4 py-3 text-slate-700 dark:text-slate-300 font-medium">Year {r.year}</td>
                  <td className="px-4 py-3 text-right text-slate-900 dark:text-white font-semibold">{disp(r.cf)}</td>
                  <td className="px-4 py-3 text-right text-slate-600 dark:text-slate-400">{r.df.toFixed(4)}</td>
                  <td className="px-4 py-3 text-right text-indigo-300 font-semibold">{disp(r.pv)}</td>
                </tr>)}
                <tr className="border-t-2 border-white/20 bg-white dark:bg-white/5"><td colSpan={3} className="px-4 py-3 font-bold text-slate-900 dark:text-white">Total PV</td><td className="px-4 py-3 text-right font-extrabold text-indigo-400">{disp(sumPV)}</td></tr>
              </tbody>
            </table>
          </div>
        </motion.div>
