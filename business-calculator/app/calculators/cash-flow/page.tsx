  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 dark:from-slate-900 via-slate-100 dark:via-slate-800 to-teal-100 dark:to-teal-950 py-10 px-4">
      <div className="max-w-7xl mx-auto mb-8 text-center">
        ...
        <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-4xl font-extrabold text-slate-900 dark:text-slate-900 dark:text-white tracking-tight mb-2">Cash Flow <span className="text-teal-400">Calculator</span></motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-slate-600 dark:text-slate-600 dark:text-slate-400 text-lg">Track your inflows and outflows to understand your financial health.</motion.p>
      </div>

      <div className="max-w-7xl mx-auto flex flex-col gap-6">
        {/* Controls */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-200 dark:border-white/10 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <label className="text-sm font-semibold text-slate-700 dark:text-slate-700 dark:text-slate-300">Input Currency</label>
            <select value={selectedInputCurrency} onChange={e => { if (availableCurrencies.includes(e.target.value as never)) setSelectedInputCurrency(e.target.value as never); }} disabled={rL} className="px-3 py-2 rounded-xl bg-white dark:bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-200 dark:border-white/10 text-slate-900 dark:text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-teal-500">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-0.5 bg-white dark:bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-200 dark:border-white/10 rounded-lg p-0.5">
              {(['monthly', 'yearly'] as const).map(p => <button key={p} onClick={() => setPeriod(p)} className={`px-3 py-1.5 text-xs rounded-md font-semibold transition-all ${period === p ? 'bg-teal-600 text-white' : 'text-slate-500 hover:text-slate-900 dark:text-slate-900 dark:text-white'}`}>{p.charAt(0).toUpperCase() + p.slice(1)}</button>)}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Inflows */}
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }} className="bg-white dark:bg-white dark:bg-white/5 backdrop-blur-xl border border-emerald-500/20 rounded-2xl shadow-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2"><TrendingUp className="w-5 h-5 text-emerald-400" /><h2 className="text-lg font-bold text-slate-900 dark:text-slate-900 dark:text-white">Cash Inflows</h2></div>
            ...
            <AnimatePresence>
              {inflows.map((e, i) => (
                <motion.div key={e.id} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex items-center gap-2 mb-3">
                  <input type="text" value={e.label} onChange={ev => updEntry(inflows, setInflows, e.id, 'label', ev.target.value)} className="flex-1 px-3 py-2 rounded-lg bg-white dark:bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-200 dark:border-white/10 text-slate-900 dark:text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500" />
                  <div className="relative w-36"><span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-600 dark:text-slate-400 text-xs">{inputSym}</span>
                    <input type="number" value={e.amount} onChange={ev => updEntry(inflows, setInflows, e.id, 'amount', ev.target.value)} placeholder="0" min={0} className="w-full pl-6 pr-2 py-2 rounded-lg bg-white dark:bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-200 dark:border-white/10 text-slate-900 dark:text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-1 focus:ring-emerald-500" /></div>
            ...
            <div className="mt-4 pt-3 border-t border-gray-200 dark:border-gray-200 dark:border-white/10 flex justify-between"><span className="text-sm font-semibold text-slate-600 dark:text-slate-600 dark:text-slate-400">Total Inflow</span><span className="text-lg font-bold text-emerald-400">{disp(totalInflow)}</span></div>
          </motion.div>

          {/* Outflows */}
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }} className="bg-white dark:bg-white dark:bg-white/5 backdrop-blur-xl border border-red-500/20 rounded-2xl shadow-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2"><TrendingDown className="w-5 h-5 text-red-400" /><h2 className="text-lg font-bold text-slate-900 dark:text-slate-900 dark:text-white">Cash Outflows</h2></div>
            ...
            <AnimatePresence>
              {outflows.map(e => (
                <motion.div key={e.id} initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="flex items-center gap-2 mb-3">
                  <input type="text" value={e.label} onChange={ev => updEntry(outflows, setOutflows, e.id, 'label', ev.target.value)} className="flex-1 px-3 py-2 rounded-lg bg-white dark:bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-200 dark:border-white/10 text-slate-900 dark:text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-1 focus:ring-red-500" />
                  <div className="relative w-36"><span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-600 dark:text-slate-400 text-xs">{inputSym}</span>
                    <input type="number" value={e.amount} onChange={ev => updEntry(outflows, setOutflows, e.id, 'amount', ev.target.value)} placeholder="0" min={0} className="w-full pl-6 pr-2 py-2 rounded-lg bg-white dark:bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-200 dark:border-white/10 text-slate-900 dark:text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-1 focus:ring-red-500" /></div>
            ...
            <div className="mt-4 pt-3 border-t border-gray-200 dark:border-gray-200 dark:border-white/10 flex justify-between"><span className="text-sm font-semibold text-slate-600 dark:text-slate-600 dark:text-slate-400">Total Outflow</span><span className="text-lg font-bold text-red-400">{disp(totalOutflow)}</span></div>
          </motion.div>
        </div>

        {/* Summary */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }} className="bg-white dark:bg-white dark:bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-gray-200 dark:border-gray-200 dark:border-white/10 shadow-sm dark:shadow-2xl rounded-2xl shadow-2xl p-6">
          <div className="flex items-center justify-between mb-2"><h2 className="text-lg font-bold text-slate-900 dark:text-slate-900 dark:text-white">Cash Flow Summary</h2><span className="text-xs text-slate-500">Updated: <span className="text-teal-300">{relTime}</span></span></div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            ...
            <div className={`${netCF >= 0 ? 'bg-emerald-500/10 border-emerald-500/20' : 'bg-red-500/10 border-red-500/20'} border rounded-xl p-4 text-center`}>
              <p className="text-xs font-semibold text-slate-700 dark:text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-1">Net Cash Flow</p>
          ...
          <div className="space-y-3 mb-4">
            {[{ label: 'Inflows', val: totalInflow, pct: (totalInflow / maxBar) * 100, color: 'bg-emerald-500', text: 'text-emerald-300' }, { label: 'Outflows', val: totalOutflow, pct: (totalOutflow / maxBar) * 100, color: 'bg-red-500', text: 'text-red-300' }].map((bar, i) => (
              <div key={bar.label}>
                ...
                <div className="h-4 bg-white dark:bg-white dark:bg-white/5 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${bar.pct}%` }} transition={{ duration: 0.8, delay: i * 0.1 }} className={`h-full ${bar.color} rounded-full`} /></div>
              </div>
            ))}
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white dark:bg-white dark:bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-gray-200 dark:border-gray-200 dark:border-white/10 shadow-sm dark:shadow-2xl rounded-2xl shadow-2xl p-6">
            <h3 className="text-base font-bold text-slate-900 dark:text-slate-900 dark:text-white mb-4">Cash Flow Table ({period})</h3>
            <div className="overflow-auto rounded-xl border border-gray-200 dark:border-gray-200 dark:border-white/10 max-h-72">
              <table className="w-full text-sm">
                <thead><tr className="bg-gray-50 dark:bg-white dark:bg-white/5 text-slate-600 dark:text-slate-600 dark:text-slate-400 font-semibold"><th className="px-4 py-3 text-left">Category</th><th className="px-4 py-3 text-right">Amount</th></tr></thead>
                <tbody>
                  ...
                  <tr className="border-t-2 border-white/20 bg-white dark:bg-white dark:bg-white/5"><td className="px-4 py-3 font-bold text-slate-900 dark:text-slate-900 dark:text-white">Net Cash Flow</td><td className={`px-4 py-3 text-right font-extrabold ${netCF >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>{netCF >= 0 ? '+' : '-'}{disp(Math.abs(netCF))}</td></tr>
                </tbody>
              </table>
            </div>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }} className="flex flex-col gap-4">
            ...
            <div className="bg-white dark:bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-200 dark:border-white/10 rounded-2xl p-5">
              <p className="text-xs font-semibold text-slate-600 dark:text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-3">Expense Breakdown</p>
              {outflows.filter(e => Number(e.amount) > 0).map((e, i) => {
                ...
                return <div key={e.id} className="mb-2"><div className="flex justify-between text-xs mb-1"><span className="text-slate-600 dark:text-slate-600 dark:text-slate-400">{e.label}</span><span className="text-slate-700 dark:text-slate-700 dark:text-slate-300">{pct.toFixed(1)}%</span></div><div className="h-2.5 bg-white dark:bg-white dark:bg-white/5 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.7, delay: i * 0.05 }} className={`h-full ${colors[i % colors.length]} rounded-full`} /></div></div>;
              })}
