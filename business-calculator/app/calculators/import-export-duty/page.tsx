  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 dark:from-slate-900 via-slate-100 dark:via-slate-800 to-violet-100 dark:to-violet-950 py-10 px-4">
      <div className="max-w-6xl mx-auto mb-8 text-center">
        ...
        <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-4xl font-extrabold text-slate-900 dark:text-slate-900 dark:text-white tracking-tight mb-2">Import / Export <span className="text-violet-400">Duty Calculator</span></motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-slate-600 dark:text-slate-600 dark:text-slate-400 text-lg">Calculate total landed cost including customs duty, taxes and charges.</motion.p>
      </div>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }} className="bg-white dark:bg-white dark:bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-gray-200 dark:border-gray-200 dark:border-white/10 shadow-sm dark:shadow-2xl rounded-2xl shadow-2xl p-6 flex flex-col gap-4">
          <div className="flex items-center justify-between"><div><h2 className="text-lg font-bold text-slate-900 dark:text-slate-900 dark:text-white">Inputs</h2><p className="text-slate-600 dark:text-slate-600 dark:text-slate-400 text-sm">Enter shipment details</p></div>
            <button onClick={() => { setProductValueRaw('500000'); setShippingRaw('20000'); setInsuranceRaw('5000'); setDutyRateRaw('10'); setHandlingRaw('0'); setPortRaw('0'); setDocRaw('0'); setTaxRateRaw('18'); }} className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-slate-700 dark:text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:text-slate-900 dark:text-white bg-white dark:bg-white dark:bg-white/5 hover:bg-white/10 border border-gray-200 dark:border-gray-200 dark:border-white/10 rounded-lg transition-all"><RotateCcw className="w-3.5 h-3.5" /> Reset</button>
          </div>
          ...
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-700 dark:text-slate-300 mb-1.5">Customs Duty Rate (%)</label>
            <div className="relative"><input type="number" value={dutyRateRaw} onChange={e => setDutyRateRaw(e.target.value)} min={0} max={200} step="0.1" className="w-full pl-4 pr-10 py-3 rounded-xl bg-white dark:bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-200 dark:border-white/10 text-slate-900 dark:text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-violet-500 transition-all" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-600 dark:text-slate-400 text-sm">%</span></div>
          </div>
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-700 dark:text-slate-300 mb-2">Additional Charges</label>
              ...
              {[{ label: 'Handling', val: handlingRaw, set: setHandlingRaw }, { label: 'Port', val: portRaw, set: setPortRaw }, { label: 'Docs', val: docRaw, set: setDocRaw }].map(f => (
                <div key={f.label}><label className="block text-xs text-slate-500 mb-1">{f.label}</label>
                  <div className="relative"><span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-600 dark:text-slate-400 text-xs">{inputSym}</span>
                    <input type="number" value={f.val} onChange={e => f.set(e.target.value)} min={0} className="w-full pl-6 pr-2 py-2 rounded-lg bg-white dark:bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-200 dark:border-white/10 text-slate-900 dark:text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-1 focus:ring-violet-500" /></div>
                </div>
              ))}
            </div>
          </div>
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-700 dark:text-slate-300 mb-2">Tax Type</label>
            <div className="grid grid-cols-3 gap-2">
              {(['GST', 'VAT', 'Other'] as const).map(t => <button key={t} onClick={() => setTaxType(t)} className={`py-2 rounded-xl border text-xs font-semibold transition-all ${taxType === t ? 'bg-violet-600 border-violet-500 text-white' : 'bg-white dark:bg-white dark:bg-white/5 border-gray-200 dark:border-gray-200 dark:border-white/10 text-slate-600 dark:text-slate-600 dark:text-slate-400 hover:border-violet-500/50'}`}>{t}</button>)}
            </div>
          </div>
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-700 dark:text-slate-300 mb-1.5">{taxType} Rate (%)</label>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="flex flex-col gap-5">
          <div className="bg-white dark:bg-white dark:bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-gray-200 dark:border-gray-200 dark:border-white/10 shadow-sm dark:shadow-2xl rounded-2xl shadow-2xl p-6">
            <div className="flex items-center justify-between mb-5"><div><h2 className="text-lg font-bold text-slate-900 dark:text-slate-900 dark:text-white">Results</h2><p className="text-slate-600 dark:text-slate-600 dark:text-slate-400 text-sm">Updated: <span className="text-violet-300">{relTime}</span></p></div>
            ...
            {result ? (
              <div className="flex flex-col gap-3">
                <div className="bg-gradient-to-r from-violet-500/20 to-purple-500/20 border border-violet-500/30 rounded-xl p-4"><p className="text-xs font-semibold text-violet-300 uppercase tracking-wider mb-1">Total Landed Cost</p><p className="text-3xl font-extrabold text-slate-900 dark:text-slate-900 dark:text-white">{dispS(result.total)}</p><p className="text-xs text-slate-600 dark:text-slate-600 dark:text-slate-400 mt-1">{disp(result.total)}</p></div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-white dark:bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-200 dark:border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-600 dark:text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">CIF Value</p><p className="text-lg font-bold text-blue-300">{dispS(result.cif)}</p></div>
                  <div className="bg-white dark:bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-200 dark:border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-600 dark:text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Import Duty</p><p className="text-lg font-bold text-amber-300">{dispS(result.duty)}</p></div>
                  <div className="bg-white dark:bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-200 dark:border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-600 dark:text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">{taxType} Amount</p><p className="text-lg font-bold text-red-300">{dispS(result.tax)}</p></div>
                  <div className="bg-white dark:bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-200 dark:border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-600 dark:text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Add. Charges</p><p className="text-lg font-bold text-slate-700 dark:text-slate-700 dark:text-slate-300">{dispS(result.additional)}</p></div>
                </div>
                <div className="bg-white dark:bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-200 dark:border-white/10 rounded-xl p-4">
                  <p className="text-xs font-semibold text-slate-600 dark:text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-3">Cost Breakdown</p>
                  {components.filter(c => c.val > 0).map((c, i) => { const pct = result.total > 0 ? (convertToINR(c.val, selectedResultCurrency as never) / result.total) * 100 : 0; const colors = ['bg-blue-500', 'bg-sky-500', 'bg-cyan-500', 'bg-amber-500', 'bg-red-500', 'bg-slate-500']; return <div key={c.label} className="mb-2"><div className="flex justify-between text-xs mb-1"><span className="text-slate-600 dark:text-slate-600 dark:text-slate-400">{c.label}</span><span className="text-slate-700 dark:text-slate-700 dark:text-slate-300">{pct.toFixed(1)}%</span></div><div className="h-2.5 bg-white dark:bg-white dark:bg-white/5 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} transition={{ duration: 0.7, delay: i * 0.05 }} className={`h-full ${colors[i % colors.length]} rounded-full`} /></div></div>; })}
                </div>
              </div>
          ...
          {result && (
            <div className="bg-white dark:bg-white dark:bg-white/5 border border-gray-200 dark:border-gray-200 dark:border-white/10 rounded-2xl overflow-hidden">
              <div className="px-5 py-3 border-b border-gray-200 dark:border-gray-200 dark:border-white/10"><p className="text-sm font-bold text-slate-900 dark:text-slate-900 dark:text-white">Cost Breakdown Table</p></div>
              <table className="w-full text-sm">
                <tbody>
                  {components.map((c, i) => <tr key={c.label} className={`border-t border-gray-100 dark:border-gray-100 dark:border-white/5 ${i % 2 === 0 ? '' : 'bg-white/[0.02]'}`}><td className="px-4 py-2.5 text-slate-700 dark:text-slate-700 dark:text-slate-300">{c.label}</td><td className="px-4 py-2.5 text-right text-slate-900 dark:text-slate-900 dark:text-white font-semibold">{fmtAmt(c.val, currSym)}</td></tr>)}
                  <tr className="border-t-2 border-white/20 bg-violet-500/5"><td className="px-4 py-3 font-bold text-slate-900 dark:text-slate-900 dark:text-white">Total Landed Cost</td><td className="px-4 py-3 text-right font-extrabold text-violet-400">{disp(result.total)}</td></tr>
                </tbody>
              </table>
            </div>
          )}
