...
export default function DepreciationCalculator() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-orange-950 py-10 px-4">
      <div className="max-w-6xl mx-auto mb-8 text-center">
        ...
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-slate-400 text-lg">Calculate asset depreciation using Straight-Line or Declining Balance methods.</motion.p>
      </div>
      <div className="max-w-6xl mx-auto flex flex-col gap-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 flex flex-col gap-5">
            <div className="flex items-center justify-between">
              <div><h2 className="text-lg font-bold text-white">Inputs</h2><p className="text-slate-400 text-sm">Enter asset details</p></div>
            ...
            <div>
              ...
              <select value={selectedInputCurrency} onChange={e => { if (availableCurrencies.includes(e.target.value as never)) setSelectedInputCurrency(e.target.value as never); }} disabled={ratesLoading} className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all">
            ...
            {[{ label: 'Asset Cost', val: costRaw, set: setCostRaw }, { label: 'Salvage Value', val: salvageRaw, set: setSalvageRaw }].map(f => (
              <div key={f.label}>
                ...
                <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-sm pointer-events-none">{inputSym}</span>
                  <input type="number" value={f.val} onChange={e => f.set(e.target.value)} min={0} className="w-full pl-8 pr-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all" /></div>
              </div>
            ))}
            <div>
              <label className="block text-sm font-semibold text-slate-300 mb-1.5">Useful Life (Years)</label>
              <input type="number" value={lifeRaw} onChange={e => setLifeRaw(e.target.value)} placeholder="e.g. 5" min={1} step={1} className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-slate-300 mb-2">Depreciation Method</label>
              <div className="grid grid-cols-2 gap-2">
                {[{ id: 'straight-line', label: 'Straight-Line' }, { id: 'declining', label: 'Declining Balance' }].map(m => (
                  <button key={m.id} onClick={() => setMethod(m.id as 'straight-line' | 'declining')} className={`py-2.5 px-3 rounded-xl border text-xs font-semibold transition-all ${method === m.id ? 'bg-orange-600 border-orange-500 text-white' : 'bg-white/5 border-white/10 text-slate-400 hover:border-orange-500/50 hover:text-slate-200'}`}>{m.label}</button>
                ))}
              </div>
            </div>
            {method === 'declining' && (
              <div>
                <label className="block text-sm font-semibold text-slate-300 mb-1.5">Depreciation Rate (%)</label>
                <div className="relative"><input type="number" value={rateRaw} onChange={e => setRateRaw(e.target.value)} min={1} max={99} className="w-full pl-4 pr-10 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-sm pointer-events-none">%</span></div>
              </div>
            )}
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="flex flex-col gap-6">
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6">
              <div className="flex items-center justify-between mb-5">
                <div><h2 className="text-lg font-bold text-white">Results</h2><p className="text-slate-400 text-sm">Updated: <span className="text-orange-300">{relTime}</span></p></div>
              ...
              <div className="mb-5"><p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Display Currency</p><div className="flex flex-wrap gap-2">{availableCurrencies.map(c => <button key={c} onClick={() => setSelectedResultCurrency(c as never)} className={`px-3 py-1 text-xs rounded-full border font-medium transition-all ${selectedResultCurrency === c ? 'bg-orange-600 border-orange-500 text-white' : 'bg-white/5 border-white/10 text-slate-400 hover:border-orange-400 hover:text-slate-200'}`}>{c}</button>)}</div></div>
              {result ? (
                <div className="grid grid-cols-1 gap-3">
                  <div className="grid grid-cols-3 gap-3">
                    <div className="bg-orange-500/10 border border-orange-500/20 rounded-xl p-3 text-center"><p className="text-[10px] font-semibold text-orange-300 uppercase tracking-wider mb-1">Year 1 Dep.</p><p className="text-base font-bold text-white">{dispShort(result.annualDep)}</p></div>
                    <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-3 text-center"><p className="text-[10px] font-semibold text-red-300 uppercase tracking-wider mb-1">Total Dep.</p><p className="text-base font-bold text-white">{dispShort(result.totalDep)}</p></div>
                    <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-3 text-center"><p className="text-[10px] font-semibold text-emerald-300 uppercase tracking-wider mb-1">Final Book Value</p><p className="text-base font-bold text-white">{dispShort(result.bookValue)}</p></div>
                  </div>
                </div>
              ) : (
        ...
        {schedule.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6">
            <h3 className="text-base font-bold text-white mb-4">Depreciation Schedule</h3>
            <div className="overflow-x-auto rounded-xl border border-white/10 max-h-80 overflow-y-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {schedule.map((row, i) => (
                    <tr key={row.year} className={`border-t border-white/5 hover:bg-white/5 transition-colors ${i % 2 === 0 ? '' : 'bg-white/[0.02]'}`}>
                      <td className="px-4 py-3 text-slate-300 font-medium">Year {row.year}</td>
