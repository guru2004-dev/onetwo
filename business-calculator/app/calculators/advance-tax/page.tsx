        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 flex flex-col gap-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-400 text-sm">Enter expected tax bounds</p>
            </div>
            <button
              onClick={handleReset}
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-2.5 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-amber-500 transition-all outline-none"
            >
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-[13px] text-slate-300 mb-1">Total Estimated Tax Liability (Yearly)</label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-sm">{inputSym}</span>
                <input type="number" min={0} value={estimatedTax} onChange={e => setEstimatedTax(e.target.value)} className="w-full pl-8 pr-3 py-3 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-amber-500 transition-all font-semibold text-lg" />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 pt-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-[13px] text-slate-300 mb-1">TDS Deducted (Est.)</label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-sm">{inputSym}</span>
                  <input type="number" min={0} value={tdsDeducted} onChange={e => setTdsDeducted(e.target.value)} className="w-full pl-8 pr-3 py-2.5 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-amber-500 transition-all font-semibold" />
                </div>
              </div>
              <div>
                <label className="block text-[13px] text-slate-300 mb-1">TCS Collected</label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-sm">{inputSym}</span>
                  <input type="number" min={0} value={tcsCollected} onChange={e => setTcsCollected(e.target.value)} className="w-full pl-8 pr-3 py-2.5 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-amber-500 transition-all font-semibold" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 h-full flex flex-col">
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-slate-400 text-sm">Updated: <span className="text-amber-300">{relTime}</span></p>
              </div>
              <button
                  ...
                  <button
                    ...
                    className={`px-3 py-1 text-xs rounded-full border font-medium transition-all ${
                      selectedResultCurrency === c
                        ...
                        : 'bg-white/5 border-white/10 text-slate-400 hover:border-amber-400 hover:text-slate-200'
                    }`}
                  >
                    {c}
                  </button>
              ...
              <div className="flex flex-col flex-1">
                <div className={`border rounded-2xl p-5 mb-5 text-center ${results.requiresAdvanceTax ? 'bg-gradient-to-r from-amber-500/10 to-orange-500/10 border-amber-500/30' : 'bg-gradient-to-r from-emerald-600/10 to-teal-600/10 border-emerald-500/30'}`}>
                  ...
                  <div className="inline-flex items-center justify-center bg-black/20 rounded-full px-4 py-1 text-sm border border-white/5">
                    <span className="text-slate-300 font-semibold mr-2">TDS/TCS Credits:</span>
                    ...
                    <table className="w-full text-sm text-left">
                      <thead className="bg-white/5 text-slate-400 font-semibold text-xs border-b border-white/10">
                        <tr>
                          ...
                          <th className="px-4 py-2.5 text-right bg-white/5">Installment</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-white/5 text-slate-300 text-xs sm:text-sm">
                        {results.installments.map((inst, i) => (
                          <tr key={i} className="hover:bg-white/5 transition-colors">
                            ...
                            <td className="px-4 py-3 text-right font-bold text-orange-300 bg-white/5">
                              {disp(inst.payable)}
                            </td>
                          </tr>
                        ))}
      ...
      {results && results.requiresAdvanceTax && (
        <div className="max-w-6xl mx-auto mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6">
            <h3 className="text-base font-bold text-white mb-6">Installment Schedule</h3>
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 flex flex-col">
            <h3 className="text-base font-bold text-white mb-2">Total Tax Liability Split</h3>
