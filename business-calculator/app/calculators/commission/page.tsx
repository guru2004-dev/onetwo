...
export default function CommissionCalculator() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-lime-950 py-10 px-4">
      <div className="max-w-6xl mx-auto mb-8 text-center">
        ...
        <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-4xl font-extrabold text-white tracking-tight mb-2">Commission <span className="text-lime-400">Calculator</span></motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-slate-400 text-lg">Calculate your commission, bonuses, and final take-home earnings.</motion.p>
      </div>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 flex flex-col gap-4">
          <div className="flex items-center justify-between"><div><h2 className="text-lg font-bold text-white">Inputs</h2></div>
            <button onClick={() => { setSalesRaw('500000'); setCommType('percentage'); setCommValueRaw('5'); setBonusRaw(''); setTaxRateRaw('10'); }} className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-slate-300 hover:text-white bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg transition-all"><RotateCcw className="w-3.5 h-3.5" /> Reset</button>
          </div>
          <div><label className="block text-sm font-semibold text-slate-300 mb-2">Input Currency</label>
            <select value={selectedInputCurrency} onChange={e => { if (availableCurrencies.includes(e.target.value as never)) setSelectedInputCurrency(e.target.value as never); }} disabled={rL} className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-lime-500 transition-all">
              {availableCurrencies.map(c => <option key={c} value={c} className="bg-slate-800">{c} ({getCurrencySymbol(c)})</option>)}
            </select>
          </div>
          <div><label className="block text-sm font-semibold text-slate-300 mb-1.5">Sales Amount</label>
            <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm">{inputSym}</span>
              <input type="number" value={salesRaw} onChange={e => setSalesRaw(e.target.value)} min={0} className="w-full pl-8 pr-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-lime-500 transition-all" /></div>
          ...
          <div><label className="block text-sm font-semibold text-slate-300 mb-2">Commission Type</label>
            <div className="grid grid-cols-2 gap-2">{[{ id: 'percentage', label: 'Percentage (%)' }, { id: 'flat', label: 'Flat Amount' }].map(t => <button key={t.id} onClick={() => setCommType(t.id as 'percentage' | 'flat')} className={`py-2.5 rounded-xl border text-xs font-semibold transition-all ${commType === t.id ? 'bg-lime-600 border-lime-500 text-white' : 'bg-white/5 border-white/10 text-slate-400 hover:border-lime-500/50'}`}>{t.label}</button>)}</div>
          </div>
          <div><label className="block text-sm font-semibold text-slate-300 mb-1.5">Commission Value {commType === 'flat' ? `(${inputSym})` : '(%)'}</label>
            <div className="relative"><input type="number" value={commValueRaw} onChange={e => setCommValueRaw(e.target.value)} min={0} max={commType === 'percentage' ? 100 : undefined} step="0.1" className="w-full pl-4 pr-10 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-lime-500 transition-all" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm">{commType === 'percentage' ? '%' : inputSym}</span></div>
          </div>
          <div><label className="block text-sm font-semibold text-slate-300 mb-1.5">Bonus Amount ({inputSym}) <span className="text-slate-500 font-normal">optional</span></label>
            <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm">{inputSym}</span>
              <input type="number" value={bonusRaw} onChange={e => setBonusRaw(e.target.value)} min={0} placeholder="0" className="w-full pl-8 pr-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-lime-500 transition-all" /></div>
          </div>
          <div><label className="block text-sm font-semibold text-slate-300 mb-1.5">Tax Rate on Commission (%) <span className="text-slate-500 font-normal">optional</span></label>
            <div className="relative"><input type="number" value={taxRateRaw} onChange={e => setTaxRateRaw(e.target.value)} min={0} max={50} step="0.1" className="w-full pl-4 pr-10 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-lime-500 transition-all" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm">%</span></div>
          </div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="flex flex-col gap-5">
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6">
            <div className="flex items-center justify-between mb-5"><div><h2 className="text-lg font-bold text-white">Results</h2><p className="text-slate-400 text-sm">Updated: <span className="text-lime-300">{relTime}</span></p></div>
            ...
            <div className="mb-5"><p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Display Currency</p><div className="flex flex-wrap gap-2">{availableCurrencies.map(c => <button key={c} onClick={() => setSelectedResultCurrency(c as never)} className={`px-3 py-1 text-xs rounded-full border font-medium transition-all ${selectedResultCurrency === c ? 'bg-lime-600 border-lime-500 text-white' : 'bg-white/5 border-white/10 text-slate-400 hover:border-lime-400'}`}>{c}</button>)}</div></div>
            {result ? (
              <div className="flex flex-col gap-3">
                <div className="bg-gradient-to-r from-lime-500/20 to-green-500/20 border border-lime-500/30 rounded-xl p-4"><p className="text-xs font-semibold text-lime-300 uppercase tracking-wider mb-1">Final Earnings</p><p className="text-3xl font-extrabold text-white">{dispS(result.finalEarnings)}</p><p className="text-xs text-slate-400 mt-1">{disp(result.finalEarnings)}</p></div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-white/5 border border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Commission</p><p className="text-xl font-bold text-lime-400">{dispS(result.commission)}</p><p className="text-xs text-slate-500">{commissionPct.toFixed(1)}% of sales</p></div>
                  <div className="bg-white/5 border border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Bonus</p><p className="text-xl font-bold text-emerald-300">{dispS(result.bonus)}</p></div>
                  <div className="bg-white/5 border border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Total Commission</p><p className="text-xl font-bold text-white">{dispS(result.totalComm)}</p></div>
                  <div className="bg-white/5 border border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Tax Deducted</p><p className="text-xl font-bold text-red-300">-{dispS(result.tax)}</p></div>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  ...
                  {[{ label: 'Final Earnings', pct: result.totalComm > 0 ? (result.finalEarnings / result.totalComm) * 100 : 100, color: 'bg-lime-500', text: 'text-lime-300' }, { label: 'Tax', pct: result.totalComm > 0 ? (result.tax / result.totalComm) * 100 : 0, color: 'bg-red-500', text: 'text-red-300' }].map((bar, i) => (
                    <div key={bar.label} className="mb-2"><div className="flex justify-between text-xs mb-1"><span className={`font-semibold ${bar.text}`}>{bar.label}</span><span className="text-slate-300">{bar.pct.toFixed(1)}%</span></div><div className="h-3 bg-white/5 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${bar.pct}%` }} transition={{ duration: 0.8, delay: i * 0.1 }} className={`h-full ${bar.color} rounded-full`} /></div></div>
                  ))}
                </div>
                  ...
                  <table className="w-full text-sm">
                    <tbody>
                      {[{ label: 'Sales Amount', val: disp(result.salesAmt) }, { label: 'Commission', val: disp(result.commission) }, { label: 'Bonus', val: disp(result.bonus) }, { label: 'Total Commission', val: disp(result.totalComm) }, { label: 'Tax Deduction', val: `-${disp(result.tax)}` }, { label: 'Final Earnings', val: disp(result.finalEarnings), highlight: true }].map((row, i) => (
                        <tr key={row.label} className={`border-t border-white/5 ${i % 2 === 0 ? '' : 'bg-white/[0.02]'} ${row.highlight ? 'bg-lime-500/5' : ''}`}>
                          <td className="px-4 py-2.5 text-slate-300 text-xs">{row.label}</td><td className={`px-4 py-2.5 text-right text-xs ${row.highlight ? 'text-lime-400 font-extrabold text-base' : 'text-white font-semibold'}`}>{row.val}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
