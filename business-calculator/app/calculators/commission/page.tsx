  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 dark:from-slate-900 via-slate-100 dark:via-slate-800 to-lime-100 dark:to-lime-950 py-10 px-4">
      <div className="max-w-6xl mx-auto mb-8 text-center">
        ...
        <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-2">Commission <span className="text-lime-400">Calculator</span></motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-slate-600 dark:text-slate-400 text-lg">Calculate your commission, bonuses, and final take-home earnings.</motion.p>
      </div>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }} className="bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-md dark:shadow-2xl rounded-2xl p-6 flex flex-col gap-4">
          <div className="flex items-center justify-between"><div><h2 className="text-lg font-bold text-slate-900 dark:text-white">Inputs</h2></div>
            <button onClick={() => { setSalesRaw('500000'); setCommType('percentage'); setCommValueRaw('5'); setBonusRaw(''); setTaxRateRaw('10'); }} className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:text-white bg-white dark:bg-white/5 hover:bg-white/10 border border-gray-200 dark:border-white/10 rounded-lg transition-all"><RotateCcw className="w-3.5 h-3.5" /> Reset</button>
          </div>
          ...
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Sales Amount</label>
            <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-400 text-sm">{inputSym}</span>
              <input type="number" value={salesRaw} onChange={e => setSalesRaw(e.target.value)} min={0} className="w-full pl-8 pr-4 py-3 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-lime-500 transition-all" /></div>
          ...
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Commission Type</label>
            <div className="grid grid-cols-2 gap-2">{[{ id: 'percentage', label: 'Percentage (%)' }, { id: 'flat', label: 'Flat Amount' }].map(t => <button key={t.id} onClick={() => setCommType(t.id as 'percentage' | 'flat')} className={`py-2.5 rounded-xl border text-xs font-semibold transition-all ${commType === t.id ? 'bg-lime-600 border-lime-500 text-white' : 'bg-white dark:bg-white/5 border-gray-200 dark:border-white/10 text-slate-600 dark:text-slate-400 hover:border-lime-500/50'}`}>{t.label}</button>)}</div>
          </div>
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Commission Value {commType === 'flat' ? `(${inputSym})` : '(%)'}</label>
            <div className="relative"><input type="number" value={commValueRaw} onChange={e => setCommValueRaw(e.target.value)} min={0} max={commType === 'percentage' ? 100 : undefined} step="0.1" className="w-full pl-4 pr-10 py-3 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-lime-500 transition-all" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-400 text-sm">{commType === 'percentage' ? '%' : inputSym}</span></div>
          </div>
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Bonus Amount ({inputSym}) <span className="text-slate-500 font-normal">optional</span></label>
            <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-400 text-sm">{inputSym}</span>
              <input type="number" value={bonusRaw} onChange={e => setBonusRaw(e.target.value)} min={0} placeholder="0" className="w-full pl-8 pr-4 py-3 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-lime-500 transition-all" /></div>
          </div>
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Tax Rate on Commission (%) <span className="text-slate-500 font-normal">optional</span></label>
            <div className="relative"><input type="number" value={taxRateRaw} onChange={e => setTaxRateRaw(e.target.value)} min={0} max={50} step="0.1" className="w-full pl-4 pr-10 py-3 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-lime-500 transition-all" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-400 text-sm">%</span></div>
          </div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="flex flex-col gap-5">
          <div className="bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-md dark:shadow-2xl rounded-2xl p-6">
            <div className="flex items-center justify-between mb-5"><div><h2 className="text-lg font-bold text-slate-900 dark:text-white">Results</h2><p className="text-slate-600 dark:text-slate-400 text-sm">Updated: <span className="text-lime-300">{relTime}</span></p></div>
            ...
            <div className="mb-5"><p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-2">Display Currency</p><div className="flex flex-wrap gap-2">{availableCurrencies.map(c => <button key={c} onClick={() => setSelectedResultCurrency(c as never)} className={`px-3 py-1 text-xs rounded-full border font-medium transition-all ${selectedResultCurrency === c ? 'bg-lime-600 border-lime-500 text-white' : 'bg-white dark:bg-white/5 border-gray-200 dark:border-white/10 text-slate-600 dark:text-slate-400 hover:border-lime-400'}`}>{c}</button>)}</div></div>
            {result ? (
              <div className="flex flex-col gap-3">
                <div className="bg-gradient-to-r from-lime-500/20 to-green-500/20 border border-lime-500/30 rounded-xl p-4"><p className="text-xs font-semibold text-lime-300 uppercase tracking-wider mb-1">Final Earnings</p><p className="text-3xl font-extrabold text-slate-900 dark:text-white">{dispS(result.finalEarnings)}</p><p className="text-xs text-slate-600 dark:text-slate-400 mt-1">{disp(result.finalEarnings)}</p></div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Commission</p><p className="text-xl font-bold text-lime-400">{dispS(result.commission)}</p><p className="text-xs text-slate-500">{commissionPct.toFixed(1)}% of sales</p></div>
                  <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Bonus</p><p className="text-xl font-bold text-emerald-300">{dispS(result.bonus)}</p></div>
                  <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Total Commission</p><p className="text-xl font-bold text-slate-900 dark:text-white">{dispS(result.totalComm)}</p></div>
                  <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-4"><p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Tax Deducted</p><p className="text-xl font-bold text-red-300">-{dispS(result.tax)}</p></div>
                </div>
                    ...
                    <tbody>
                      {[{ label: 'Sales Amount', val: disp(result.salesAmt) }, { label: 'Commission', val: disp(result.commission) }, { label: 'Bonus', val: disp(result.bonus) }, { label: 'Total Commission', val: disp(result.totalComm) }, { label: 'Tax Deduction', val: `-${disp(result.tax)}` }, { label: 'Final Earnings', val: disp(result.finalEarnings), highlight: true }].map((row, i) => (
                        <tr key={row.label} className={`border-t border-gray-100 dark:border-gray-100 dark:border-white/5 ${i % 2 === 0 ? '' : 'bg-white/[0.02]'} ${row.highlight ? 'bg-lime-500/5' : ''}`}>
                          <td className="px-4 py-2.5 text-slate-700 dark:text-slate-300 text-xs">{row.label}</td><td className={`px-4 py-2.5 text-right text-xs ${row.highlight ? 'text-lime-400 font-extrabold text-base' : 'text-slate-900 dark:text-white font-semibold'}`}>{row.val}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
