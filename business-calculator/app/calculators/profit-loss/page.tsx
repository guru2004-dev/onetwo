  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 py-10 px-4">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* LEFT — INPUTS */}
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 flex flex-col gap-5">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-white">Cost & Sales Details</h2>
              <p className="text-slate-400 text-sm">Enter your trade values</p>
            </div>
            <button
              onClick={handleReset}
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all outline-none"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-semibold text-slate-300 mb-1.5">Cost Price (CP)</label>
              <div className="relative">
                ...
                <input
                  ...
                  min={0}
                  step="any"
                  className="w-full pl-9 pr-4 py-3 rounded-xl bg-slate-900/50 border border-white/10 text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all"
                />
              </div>
            </div>
            <div>
              <label className="block text-sm font-semibold text-slate-300 mb-1.5">Selling Price (SP)</label>
              <div className="relative">
                <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-sm pointer-events-none">{inputSym}</span>
                <input
                  type="number"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <div className="relative">
                <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-sm pointer-events-none"><Package className="w-4 h-4"/></span>
                <input
                  type="number"
                  value={quantity}
                  ...
                  min={1}
                  step="1"
                  className="w-full pl-10 pr-4 py-3 rounded-xl bg-slate-900/50 border border-white/10 text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all"
                />
              </div>
            </div>
            <div>
              <label className="block text-sm font-semibold text-slate-300 mb-1.5">Additional Costs</label>
              <div className="relative">
                <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 font-bold text-sm pointer-events-none">{inputSym}</span>
                <input
                  type="number"
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="text-lg font-bold text-white">Results Analysis</h2>
            ...
            {results ? (
              <>
                <div className={`border rounded-2xl p-5 mb-5 text-center ${results.isProfit ? 'bg-gradient-to-r from-emerald-600/20 to-teal-600/20 border-emerald-500/30' : 'bg-gradient-to-r from-red-600/20 to-orange-600/20 border-red-500/30'}`}>
                  ...
                  <p className="text-5xl font-extrabold text-white tracking-tight mb-1">{disp(results.profitLoss)}</p>
                  <div className="inline-flex items-center justify-center bg-black/20 rounded-full px-3 py-0.5 text-sm">
                ...
                <div className="grid grid-cols-2 gap-3 mb-5">
                  <div className="rounded-xl border bg-slate-800/50 border-white/5 p-4 flex flex-col gap-2">
                    <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-slate-400">
                      <Layers className="w-4 h-4" /> Total Cost
                    </div>
                    <p className="font-bold text-xl text-white">{disp(results.totalCost)}</p>
                  </div>
                  <div className="rounded-xl border bg-slate-800/50 border-white/5 p-4 flex flex-col gap-2">
                    <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-slate-400">
                      <DollarSign className="w-4 h-4" /> Total Revenue
                    </div>
                    <p className="font-bold text-xl text-white">{disp(results.totalSelling)}</p>
                  </div>
                </div>
                ...
                <div className="border border-white/5 rounded-xl overflow-hidden mb-5">
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-white/5 text-slate-300">
                      ...
                      <tr className="hover:bg-white/5 transition-colors text-xs sm:text-sm">
                        <td className="px-4 py-3 font-medium text-white">Revenue (SP × {quantity})</td>
                        <td className="px-4 py-3 text-right font-medium text-white">{disp(Number(sellingPrice) * Number(quantity))}</td>
                      </tr>
      ...
      {results && (
        <div className="max-w-6xl mx-auto mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6">
            <h3 className="text-base font-bold text-white mb-6">Financial Comparison</h3>
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 flex flex-col">
            <h3 className="text-base font-bold text-white mb-2">Cost vs Return Split</h3>
