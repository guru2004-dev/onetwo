      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 flex flex-col gap-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-white">Revenue & COGS</h2>
              <p className="text-slate-400 text-sm">Enter sales and direct production costs</p>
            </div>
            <button
              onClick={handleReset}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-[13px] text-slate-300 mb-1">Gross Sales</label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-sm">{inputSym}</span>
                  <input type="number" min={0} value={grossSales} onChange={e => setGrossSales(e.target.value)} className="w-full pl-8 pr-3 py-2.5 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all font-semibold" />
                </div>
              </div>
              <div>
                <label className="block text-[13px] text-slate-300 mb-1">Sales Returns & Discounts</label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-sm">{inputSym}</span>
                  <input type="number" min={0} value={salesReturns} onChange={e => setSalesReturns(e.target.value)} className="w-full pl-8 pr-3 py-2.5 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all font-semibold" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4 pt-2">
            <h3 className="text-sm font-bold text-red-400 uppercase tracking-wider flex items-center gap-2 border-b border-white/10 pb-2">
              <MinusCircle className="w-4 h-4"/> Cost of Goods Sold (COGS)
            </h3>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-[13px] text-slate-300 mb-1">Direct Materials</label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-sm">{inputSym}</span>
                  <input type="number" min={0} value={directMaterials} onChange={e => setDirectMaterials(e.target.value)} className="w-full pl-8 pr-3 py-2.5 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all font-semibold" />
                </div>
              </div>
              <div>
                <label className="block text-[13px] text-slate-300 mb-1">Direct Labor</label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-sm">{inputSym}</span>
                  <input type="number" min={0} value={directLabor} onChange={e => setDirectLabor(e.target.value)} className="w-full pl-8 pr-3 py-2.5 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all font-semibold" />
                </div>
              </div>
              <div className="sm:col-span-2">
                <label className="block text-[13px] text-slate-300 mb-1">Manufacturing Overhead</label>
                <div className="relative">
                  <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-sm">{inputSym}</span>
                  <input type="number" min={0} value={manufacturingOverhead} onChange={e => setManufacturingOverhead(e.target.value)} className="w-full pl-8 pr-3 py-2.5 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all font-semibold" />
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 h-full flex flex-col">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="text-lg font-bold text-white">Profitability Analysis</h2>
              ...
              <div className="flex flex-col flex-1">
                <div className={`border rounded-2xl p-5 mb-5 text-center ${results.grossProfit >= 0 ? 'bg-gradient-to-r from-emerald-600/10 to-teal-600/10 border-emerald-500/30' : 'bg-gradient-to-r from-red-600/10 to-rose-600/10 border-red-500/30'}`}>
                  ...
                  <div className="flex items-center justify-center gap-2">
                    <div className="bg-black/20 rounded-full px-3 py-1 text-xs border border-white/5">
                      <span className="text-slate-300 font-semibold mr-1">Margin:</span>
                      <span className={results.grossMargin > 0 ? "text-emerald-400 font-bold" : "text-red-400 font-bold"}>
                        {results.grossMargin.toFixed(2)}%
                      </span>
                    </div>
                    <div className="bg-black/20 rounded-full px-3 py-1 text-xs border border-white/5">
                      <span className="text-slate-300 font-semibold mr-1">Markup:</span>
                ...
                <div className="grid grid-cols-2 gap-3 mb-5">
                  <div className="rounded-xl border bg-slate-800/50 border-white/5 p-4 flex flex-col gap-1">
                    ...
                    <p className="font-bold text-lg text-white">{disp(results.netSales)}</p>
                  </div>
                  <div className="rounded-xl border bg-slate-800/50 border-white/5 p-4 flex flex-col gap-1">
                    <div className="text-[10px] font-semibold uppercase tracking-wider text-red-400">Total COGS</div>
                    <p className="font-bold text-lg text-white">{disp(results.totalCogs)}</p>
                  </div>
                </div>
                  ...
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-white/5 text-slate-300 text-sm">
                      ...
                      <tr className="font-bold bg-white/5">
                        <td className="px-4 py-3 text-white">Gross Profit</td>
                        <td className="px-4 py-3 text-right text-white">{results.grossProfit < 0 ? '-' : ''}{disp(results.grossProfit)}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
      ...
      {results && (
        <div className="max-w-6xl mx-auto mt-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6">
            <h3 className="text-base font-bold text-white mb-6">Revenue vs Cost</h3>
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 flex flex-col">
            <h3 className="text-base font-bold text-white mb-2">COGS Composition</h3>
