      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 flex flex-col gap-6">
          <div className="flex items-center justify-between border-b border-white/10 pb-4">
            <div>
              <h2 className="text-lg font-bold text-white">Appliance Details</h2>
              <p className="text-slate-400 text-sm">Input power specifications</p>
            </div>
            <button
              onClick={handleReset}
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-2.5 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-all outline-none"
            >
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-bold text-yellow-400 uppercase tracking-wider flex items-center gap-2 border-b border-white/10 pb-2">
              <Battery className="w-4 h-4"/> Hardware Data
            </h3>
            
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-[13px] text-slate-300 mb-1">Appliance Power</label>
                <div className="flex relative">
                   <input type="number" min={0} value={devicePower} onChange={e => setDevicePower(e.target.value)} className="w-2/3 px-4 py-2.5 rounded-l-xl bg-slate-900/50 border border-white/10 border-r-0 text-white focus:outline-none focus:border-yellow-500 transition-all font-semibold" />
                   <select value={powerUnit} onChange={e => setPowerUnit(e.target.value)} className="w-1/3 px-2 py-2.5 rounded-r-xl bg-slate-800 border border-white/10 text-slate-300 focus:outline-none focus:border-yellow-500 transition-all">
              ...
              <div>
                <label className="block text-[13px] text-slate-300 mb-1">Usage (Hours per Day)</label>
                <div className="relative">
                  <input type="number" min={0} max={24} value={hoursPerDay} onChange={e => setHoursPerDay(e.target.value)} className="w-full px-4 py-2.5 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-all font-semibold" />
                </div>
              </div>
            </div>

            <div>
              <label className="block text-[13px] text-slate-300 mb-1">Electricity Rate (per kWh)</label>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 text-sm">{inputSym}</span>
                <input type="number" min={0} step="any" value={ratePerKwh} onChange={e => setRatePerKwh(e.target.value)} className="w-full pl-8 pr-3 py-2.5 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-yellow-500 transition-all font-semibold" />
              </div>
            </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 h-full flex flex-col justify-center gap-4">
                  ...
                  <button
                    ...
                    className={`px-3 py-1 text-xs rounded-full border font-medium transition-all ${
                      selectedResultCurrency === c
                        ...
                        : 'bg-white/5 border-white/10 text-slate-400 hover:border-yellow-400 hover:text-slate-200'
                    }`}
                  >
                    {c}
                  </button>
            ...
            {results ? (
              <>
                <div className="border border-white/10 bg-gradient-to-r from-yellow-900/30 to-amber-900/30 rounded-xl p-5 shadow-inner text-center">
                  <p className="text-sm text-yellow-300 font-semibold mb-1 uppercase tracking-widest">Monthly Estimated Cost</p>
                  <p className="text-5xl font-extrabold text-white mb-2">{disp(results.monthlyCost)}</p>
                  <div className="inline-flex bg-black/20 rounded-full px-4 py-1 text-sm border border-white/5">
                    <span className="text-slate-300">Energy Run Rate:</span>
                    <span className="text-yellow-400 font-bold ml-2">{results.monthlyKwh.toFixed(1)} kWh/mo</span>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-3 mt-2">
                  <div className="rounded-xl border bg-slate-800/50 border-white/5 p-4 flex flex-col items-center text-center">
                    <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Daily Cost</div>
                    <p className="font-bold text-lg text-white mb-1">{disp(results.dailyCost)}</p>
                    <p className="text-xs text-yellow-500/70">{results.dailyKwh.toFixed(2)} kWh</p>
                  </div>
                  <div className="rounded-xl border bg-slate-800/50 border-white/5 p-4 flex flex-col items-center text-center">
                    <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Yearly Cost</div>
                    <p className="font-bold text-lg text-white mb-1">{disp(results.yearlyCost)}</p>
      ...
      {results && (
        <div className="max-w-6xl mx-auto mt-6">
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6">
            <h3 className="text-base font-bold text-white mb-6">Financial Scale of Operation</h3>
