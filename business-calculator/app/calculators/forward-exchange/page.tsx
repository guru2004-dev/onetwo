  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 dark:from-slate-900 via-slate-100 dark:via-slate-800 to-cyan-100 dark:to-cyan-950 py-10 px-4">
      <div className="max-w-6xl mx-auto mb-8 text-center">
        ...
        <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-2">Forward Exchange <span className="text-cyan-400">Calculator</span></motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-slate-600 dark:text-slate-400 text-lg">Calculate forward exchange rates for future currency transactions.</motion.p>
      </div>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }} className="bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-md dark:shadow-2xl rounded-2xl p-6 flex flex-col gap-4">
          ...
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2">Currency Pair</label>
            <div className="flex items-center gap-2">
              ...
              <select value={quoteCcy} onChange={e => setQuoteCcy(e.target.value)} className="flex-1 px-4 py-2.5 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-cyan-500 transition-all">{CURRENCIES.map(c => <option key={c} value={c} className="bg-white dark:bg-slate-800">{c}</option>)}</select>
            </div>
          </div>
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Spot Rate (1 {baseCcy} = ? {quoteCcy})</label>
            <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-400 text-sm">{quoteCcySym}</span>
              <input type="number" value={spotRaw} onChange={e => setSpotRaw(e.target.value)} min={0} step="0.001" className="w-full pl-8 pr-4 py-3 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-cyan-500 transition-all" /></div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Domestic Interest Rate</label>
              <div className="relative"><input type="number" value={domRateRaw} onChange={e => setDomRateRaw(e.target.value)} min={0} max={50} step="0.1" className="w-full pl-4 pr-10 py-3 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-cyan-500 transition-all" /><span className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-400 text-sm">%</span></div>
            </div>
            <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Foreign Interest Rate</label>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5"><label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Contract Duration</label>
              <div className="flex items-center gap-0.5 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-lg p-0.5">{(['days', 'months', 'years'] as const).map(u => <button key={u} onClick={() => setDurationUnit(u)} className={`px-2.5 py-1 text-[11px] rounded-md font-semibold transition-all ${durationUnit === u ? 'bg-cyan-600 text-white' : 'text-slate-500 hover:text-slate-900 dark:text-white'}`}>{u.charAt(0).toUpperCase() + u.slice(1)}</button>)}</div>
            </div>
          ...
          <div><label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Contract Amount ({baseCcy}) <span className="text-slate-500 font-normal">optional</span></label>
            <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-600 dark:text-slate-400 text-sm">{baseCcySym}</span>
              <input type="number" value={amountRaw} onChange={e => setAmountRaw(e.target.value)} min={0} placeholder="e.g. 100000" className="w-full pl-8 pr-4 py-3 rounded-xl bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-cyan-500 transition-all" /></div>
          </div>
          ...
          <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-3 mt-auto"><p className="text-xs font-mono text-slate-600 dark:text-slate-400">F = S × (1 + rd·t) / (1 + rf·t)</p></div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="flex flex-col gap-6">
          <div className="bg-white dark:bg-white/5 backdrop-blur-xl border border-gray-200 dark:border-white/10 shadow-md dark:shadow-2xl rounded-2xl p-6">
            <div className="flex items-center justify-between mb-5"><div><h2 className="text-lg font-bold text-slate-900 dark:text-white">Results</h2><p className="text-slate-600 dark:text-slate-400 text-sm">Updated: <span className="text-cyan-300">{relTime}</span></p></div>
              ...
              <div className="flex flex-col gap-3">
                <div className="bg-gradient-to-r from-cyan-500/20 to-sky-500/20 border border-cyan-500/30 rounded-xl p-5">
                  <div className="flex items-center justify-between mb-3">
                    <div><p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Spot Rate</p><p className="text-2xl font-bold text-slate-700 dark:text-slate-300">{quoteCcySym}{Number(spotRaw).toFixed(4)}</p></div>
                    <div className="text-right"><p className="text-xs font-semibold text-slate-600 dark:text-slate-400 uppercase tracking-wider mb-1">Forward Rate</p><p className="text-2xl font-bold text-cyan-300">{quoteCcySym}{result.forwardRate.toFixed(4)}</p></div>
                  </div>
                ...
                <div className="bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl p-4">
                  ...
                  {[{ label: `Spot (${baseCcy}/${quoteCcy})`, val: Number(spotRaw), max: Math.max(Number(spotRaw), result.forwardRate), color: 'bg-blue-500', text: 'text-blue-300' }, { label: `Forward Rate`, val: result.forwardRate, max: Math.max(Number(spotRaw), result.forwardRate), color: 'bg-cyan-500', text: 'text-cyan-300' }].map((bar, i) => (
                    <div key={bar.label} className="mb-2">
                      <div className="flex justify-between text-xs mb-1"><span className={`font-semibold ${bar.text}`}>{bar.label}</span><span className="text-slate-700 dark:text-slate-300">{bar.val.toFixed(4)}</span></div>
                      <div className="h-3 bg-white dark:bg-white/5 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${(bar.val / bar.max) * 100}%` }} transition={{ duration: 0.8, delay: i * 0.1 }} className={`h-full ${bar.color} rounded-full`} /></div>
                    </div>
                  ))}
                </div>
              </div>
