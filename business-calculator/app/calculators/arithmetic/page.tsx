      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 flex flex-col gap-6">
          <div className="flex items-center justify-between border-b border-white/10 pb-4">
            <div>
              <h2 className="text-lg font-bold text-white">Equation Setup</h2>
              <p className="text-slate-400 text-sm">Define your parameters</p>
            </div>
            <button
              onClick={handleReset}
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-[13px] text-slate-300 mb-1">First Number (X)</label>
                <div className="relative">
                   <input type="number" step="any" value={number1} onChange={e => setNumber1(e.target.value)} className="w-full px-4 py-2.5 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all font-semibold" />
                </div>
              </div>
              
              <div>
                <label className="block text-[13px] text-slate-300 mb-1">Second Number (Y)</label>
                <div className="relative">
                  <input type="number" step="any" value={number2} onChange={e => setNumber2(e.target.value)} className="w-full px-4 py-2.5 rounded-xl bg-slate-900/50 border border-white/10 text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all font-semibold" />
                </div>
              </div>
            </div>

            <div>
              <label className="block text-[13px] text-slate-300 mb-1">Operation Type</label>
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center justify-center p-3 rounded-xl border transition-all ${
                        operation === op.val 
                          ...
                          : 'bg-slate-900/40 border-white/5 text-slate-400 hover:border-blue-500/50 hover:text-white'
                      }`}
                    >
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 h-full flex flex-col">
            <div className="flex items-center justify-between mb-5">
              <div>
                <h2 className="text-lg font-bold text-white">Calculation Output</h2>
              </div>
            </div>
            ...
            {results && !isNaN(results.result) ? (
              <div className="flex flex-col flex-1">
                <div className="border rounded-2xl p-5 mb-5 text-center bg-gradient-to-r from-blue-600/10 to-indigo-600/10 border-blue-500/30">
                  ...
                  <p className={`text-6xl font-extrabold ${!isFinite(results.result) ? 'text-rose-400' : 'text-white'} tracking-tight mb-4 break-all`}>
                    {Number(results.result.toPrecision(10))}
                  </p>
                  
                  <div className="inline-flex items-center justify-center bg-slate-900 border border-white/10 rounded-xl px-5 py-3 w-full shadow-inner overflow-hidden">
                     <span className="text-slate-400 font-mono text-sm mr-3">EQ:</span>
                ...
                <div className="grid grid-cols-2 gap-3 mb-5">
                  <div className="rounded-xl border bg-slate-800/50 border-white/5 p-4 flex flex-col gap-1 relative overflow-hidden">
                    ...
                    <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Operand X</div>
                    <p className="font-bold text-lg text-white truncate">{number1}</p>
                  </div>
                  <div className="rounded-xl border bg-slate-800/50 border-white/5 p-4 flex flex-col gap-1 relative overflow-hidden">
                    <div className="absolute top-0 right-0 h-full w-1 bg-slate-500" />
                    <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-400">Operand Y</div>
                    <p className="font-bold text-lg text-white truncate">{number2}</p>
                  </div>
                </div>
      ...
      {results && !isNaN(results.result) && isFinite(results.result) && barData.some(d => d.value !== 0) && (
        <div className="max-w-6xl mx-auto mt-6">
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6">
            <h3 className="text-base font-bold text-white mb-6">Relative Scaling</h3>
