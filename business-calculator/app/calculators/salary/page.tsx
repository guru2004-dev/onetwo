  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 py-10 px-4">
      <div className="max-w-6xl mx-auto mb-8 text-center">
        ...
        <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="text-4xl font-extrabold text-white tracking-tight mb-2">Salary <span className="text-indigo-400">Calculator</span></motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-slate-400 text-lg">Calculate gross and net take-home salary after taxes and deductions.</motion.p>
      </div>
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="flex flex-col gap-6">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6 flex flex-col gap-4">
            <div className="flex items-center justify-between"><div><h2 className="text-lg font-bold text-white">Earnings</h2></div>
              <button onClick={() => { setBasicSalaryRaw('50000'); setAllowances([mkEntry('HRA'), mkEntry('Special Allowance')]); setDeductions([mkEntry('Provident Fund (PF)'), mkEntry('Professional Tax'), mkEntry('Income Tax')]); }} className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-slate-300 hover:text-white bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg transition-all"><RotateCcw className="w-3.5 h-3.5" /> Reset</button>
            </div>
            ...
            <div><label className="block text-sm font-semibold text-slate-300 mb-1.5">Basic Salary</label>
              <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-sm">{inputSym}</span>
                <input type="number" value={basicSalaryRaw} onChange={e => setBasicSalaryRaw(e.target.value)} min={0} className="w-full pl-8 pr-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all" /></div>
            </div>
            <div>
              <div className="flex items-center justify-between mb-2"><label className="text-sm font-semibold text-slate-300">Allowances</label>
              ...
              <AnimatePresence>
                {allowances.map((a, idx) => (
                  <motion.div key={a.id} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="flex items-center gap-2 mb-2">
                    <input type="text" value={a.name} onChange={e => updEntry(allowances, setAllowances, a.id, 'name', e.target.value)} className="flex-1 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500" />
                    <div className="relative w-32"><span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-400 text-xs">{inputSym}</span>
                      <input type="number" value={a.amount} onChange={e => updEntry(allowances, setAllowances, a.id, 'amount', e.target.value)} placeholder="0" className="w-full pl-6 pr-2 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500" /></div>
          ...
          {/* Deductions */}
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-white">Deductions</h2>
            ...
            <AnimatePresence>
              {deductions.map((d, idx) => (
                <motion.div key={d.id} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="flex items-center gap-2 mb-2">
                  <input type="text" value={d.name} onChange={e => updEntry(deductions, setDeductions, d.id, 'name', e.target.value)} className="flex-1 px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:ring-1 focus:ring-red-500" />
                  <div className="relative w-32"><span className="absolute left-2 top-1/2 -translate-y-1/2 text-slate-400 text-xs">{inputSym}</span>
                    <input type="number" value={d.amount} onChange={e => updEntry(deductions, setDeductions, d.id, 'amount', e.target.value)} placeholder="0" className="w-full pl-6 pr-2 py-2 rounded-lg bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:ring-1 focus:ring-red-500" /></div>
          ...
          <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl p-6">
            ...
            {result ? (
              <div className="flex flex-col gap-3">
                <div className="bg-gradient-to-r from-indigo-500/20 to-blue-500/20 border border-indigo-500/30 rounded-xl p-4"><p className="text-xs font-semibold text-indigo-300 uppercase tracking-wider mb-1">Net Take-Home Salary</p><p className="text-3xl font-extrabold text-white">{dispS(result.netSalary)}</p><p className="text-xs text-slate-400 mt-1">{disp(result.netSalary)}</p></div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-white/5 border border-white/10 rounded-xl p-3"><p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-1">Gross Salary</p><p className="text-lg font-bold text-emerald-300">{dispS(result.grossSalary)}</p></div>
                  <div className="bg-white/5 border border-white/10 rounded-xl p-3"><p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-1">Total Deductions</p><p className="text-lg font-bold text-red-300">-{dispS(result.totalDeductions)}</p></div>
                  <div className="bg-white/5 border border-white/10 rounded-xl p-3"><p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-1">Basic</p><p className="text-base font-bold text-sky-300">{dispS(result.basic)}</p></div>
                  <div className="bg-white/5 border border-white/10 rounded-xl p-3"><p className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider mb-1">Allowances</p><p className="text-base font-bold text-blue-300">+{dispS(result.totalAllowances)}</p></div>
                </div>
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">Salary Distribution</p>
                  {[{ label: 'Basic', val: result.basic, color: 'bg-sky-500', text: 'text-sky-300' }, { label: 'Allowances', val: result.totalAllowances, color: 'bg-blue-500', text: 'text-blue-300' }, { label: 'Deductions', val: result.totalDeductions, color: 'bg-red-500', text: 'text-red-300' }, { label: 'Net Pay', val: result.netSalary, color: 'bg-indigo-500', text: 'text-indigo-300' }].map((bar, i) => {
                    ...
                    return <div key={bar.label} className="mb-2"><div className="flex justify-between text-xs mb-1"><span className={`font-semibold ${bar.text}`}>{bar.label}</span><span className="text-slate-300">{pct.toFixed(1)}% of Gross</span></div><div className="h-2.5 bg-white/5 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${Math.min(pct, 100)}%` }} transition={{ duration: 0.7, delay: i * 0.05 }} className={`h-full ${bar.color} rounded-full`} /></div></div>;
                  })}
                </div>
                <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
                  <table className="w-full text-sm">
                    <tbody>
                      <tr className="bg-emerald-500/10 border-b border-white/10"><td colSpan={2} className="px-4 py-2 font-bold text-emerald-400 uppercase text-xs">Gross Earnings</td></tr>
                      <tr className="border-b border-white/5"><td className="px-4 py-2.5 text-slate-300 text-xs">Basic Salary</td><td className="px-4 py-2.5 text-right text-xs text-white">{disp(result.basic)}</td></tr>
                      {allowances.map(a => <tr key={a.id} className="border-b border-white/5 bg-white/[0.02]"><td className="px-4 py-2.5 text-slate-300 text-xs">{a.name}</td><td className="px-4 py-2.5 text-right text-xs text-emerald-300">+{disp(convertToINR(Number(a.amount) || 0, selectedInputCurrency))}</td></tr>)}
                      <tr className="border-b-2 border-white/20"><td className="px-4 py-2.5 font-bold text-slate-300 text-xs">Total Gross</td><td className="px-4 py-2.5 text-right text-sm font-bold text-emerald-400">{disp(result.grossSalary)}</td></tr>
                      <tr className="bg-red-500/10 border-b border-white/10"><td colSpan={2} className="px-4 py-2 font-bold text-red-400 uppercase text-xs">Deductions</td></tr>
                      {deductions.map(d => <tr key={d.id} className="border-b border-white/5 bg-white/[0.02]"><td className="px-4 py-2.5 text-slate-300 text-xs">{d.name}</td><td className="px-4 py-2.5 text-right text-xs text-red-300">-{disp(convertToINR(Number(d.amount) || 0, selectedInputCurrency))}</td></tr>)}
                      <tr className="border-b-2 border-white/20"><td className="px-4 py-2.5 font-bold text-slate-300 text-xs">Total Deductions</td><td className="px-4 py-2.5 text-right text-sm font-bold text-red-400">{disp(result.totalDeductions)}</td></tr>
                      <tr className="bg-indigo-500/20"><td className="px-4 py-4 font-bold text-white">Net Take-Home Salary</td><td className="px-4 py-4 text-right text-lg font-extrabold text-indigo-300">{disp(result.netSalary)}</td></tr>
                    </tbody>
                  </table>
                </div>
              </div>
